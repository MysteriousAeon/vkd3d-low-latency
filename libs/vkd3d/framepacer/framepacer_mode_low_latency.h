   ...
   */
    ...
    public:
        ...
        : FramePacerMode(mode, mode == LOW_LATENCY ? "low-latency" : "low-latency-vrr", storage, frameSync, firstFrameId),
            m_lowLatencyOffset(getLowLatencyOffset()), m_device(device) {
            INFO( "lowLatencyOffset: %" PRIi32 "\n", m_lowLatencyOffset );
...
//          { m_gpuProgress.notifyQueueSubmit( frameId, t ); }
        ...
        void finishRender(uint64_t generation, uint64_t frameId) override {
            ...
            if (m->start == time_point{}) {
                ...
                ERR( "m->start not set, skipping frame analysis \n" );
                return;
            }
            ...
            CalibratedDeviceTimestamps::Calibration calibration = m_device->m_calibratedDeviceTimestamps.getCalibration();
...
//            }
    ...
    private:
        ...
        int32_t getCpuDelay( const SyncProps& props, const InFlight& inFlight, time_point now, time_point lastStart ) const {
            ...
            return cpuDelay + m_lowLatencyOffset - 50;
...
//            return targetVBlank - expectedFrameLatency;
        ...
        static int32_t getLowLatencyOffset() {
            char env[8];
            if (vkd3d_get_env_var("VKD3D_LOW_LATENCY_OFFSET", env, sizeof(env))) {
                unsigned long offset = strtoul(env, NULL, 0);
                offset = std::min( offset, 500UL );
                return offset;
            }
            // let us actually default to an offset of 100 microseconds
            // since we are currently still missing to measure the blit
            return 100;
        }
